...
def calc_norm_stats(cfg, data_src, n_stats=10000):
    ...
    def data_for_stats(data_src):
        # LOO-CV（Leave-One-Out Cross Validation）の場合は全データを統計計算に使用
        if data_src.loocv:
            return data_src
        # それ以外（train/val/test 分割）の場合は fold 0（training set）のみを対象にする
